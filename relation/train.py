...
def runEpoch(model, loader, optimizer, criterion, epoch, validate=False, rel_iou_threshold=0):
    losses = []
    bg_acc = []
    bg_acc2 = []
    bg_losses = []
    ...
    for batch in loader:
        for data in batch:
            ...
            if len(train_rel_pos) > 0:
                ...
                bg_label_0 = torch.ones((rel_pos.shape[0],1))
                #bg_label_1 = torch.zeros((rel_neg.shape[0],1))
                ...
                bg_out = model(att_feats, w2v, rel_sample, iou_sample)
                ...
                hit = (bg_out[:sample_size]>0).sum()+(bg_out[sample_size:]<0).sum()
                acc2 = float((bg_out[:sample_size]>0).sum().item())/sample_size
                ...
                # loader.set_description('Ep{}:bg={:.4f},r={:.4f},acc={:.1f},{:.1f}'.format(epoch, np.average(bg_losses),np.average(rel_losses), np.average(bg_acc)*100, np.average(bg_acc2)*100))
                loader.set_description('Ep{}:bg={:.4f},acc={:.1f},{:.1f}'.format(epoch, np.average(bg_losses),np.average(bg_acc)*100, np.average(bg_acc2)*100))
...
def train(args, dataloader, model):
    ...
    stepLR = optim.lr_scheduler.StepLR(optimizer, step_size=args.lr_decay_step, gamma=args.lr_decay_rate)
    ...
    for epoch in range(args.epochs):
        ...
        model.train()
        runEpoch(model, tq, optimizer, criterion, epoch+1,
            validate=False, rel_iou_threshold=rel_iou_threshold)
        print('Validate')
        tq = tqdm(dataloader.valloader)
        model.eval()
        bg_losses, rel_losses, bg_acc, bg_acc2 = runEpoch(model, tq, None, criterion, epoch+1,
            validate=True, rel_iou_threshold=rel_iou_threshold)
        bg_acc2 = np.average(bg_acc2)*100
        if bg_acc2 > best_acc:
            best_acc = bg_acc2
            torch.save(model.cpu().state_dict(), args.model_path+'/ep{}-acc{:.2f}.pt'.format(epoch+1,
                bg_acc2))
            model.cuda()
        stepLR.step()
