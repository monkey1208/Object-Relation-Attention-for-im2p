...
from relation.iteration import rel_iteration, avg_features
import ipdb
...
def language_eval(dataset, preds, model_id, split):
    ...
    cache_path = os.path.join('test_results/', model_id + '_' + split + '.json')
...
def eval_split(model, gcn_model, rel_model, avg_feat, crit, loader, eval_kwargs={}):
    ...
    loss_evals = 1e-8
    predictions = []
    fout = open('valid.csv', 'w')
    fout.write('image_id,paragraph\n')
    while True:
