...
class RelationDataset(data.Dataset):
    def get_para_idx(self, fname):
        import json
        f = open(fname, 'r')
        para = json.load(f)
        para_id = [i['image_id'] for i in para]
        return para_id
    def __init__(self, opt):
        ...
        self.para_json = getattr(opt, 'para_json', None)
        self.para_id = self.get_para_idx(self.para_json)
        self.info = json.load(open(self.opt.input_json))
        self.input_fc_dir = self.opt.input_fc_dir
        self.input_att_dir = self.opt.input_att_dir
        self.input_box_dir = self.opt.input_box_dir
        self.use_cat_feat = getattr(opt, 'use_cat_feat', False)
    def __getitem__(self, index):
        """This function returns a tuple that is further passed to collate_fn
        """
        ix = index #self.split_ix[index]
        if self.use_att:
            features = np.load(os.path.join(self.input_att_dir, str(self.info['images'][ix]['id']) + '.npz'))
            if self.use_cat_feat:
            ...
            # boxes coords
            box_coord = features['boxes']
            confidence = features['confidence']
            W = features['image_w']
            H = features['image_h']
            classes = features['predict_class']
        ...
        return {
            'fc':np.load(os.path.join(self.input_fc_dir, str(self.info['images'][ix]['id']) + '.npy')),
                'att':att_feat,
                'boxes':box_coord,
                'conf':confidence,
                'image_id':image_id,
...
class RelationDataLoader(data.DataLoader):
    def __init__(self, dataset, opt, batch_size, shuffle=False, num_workers=4,
        collate_fn=None):
        super(RelationDataLoader, self).__init__(dataset, batch_size=batch_size,
            shuffle=shuffle,
        ...
        self.id2ix = {}
        for idx, inf in enumerate(self.info['images']):
            self.id2ix[inf['id']] = idx
        self.idx2sg = [0]*len(self.id2ix)
        sg = getattr(opt, 'sg_json', None)
        sg = self.read_sg(sg)
        for _sg in sg:
            im_id = _sg['image_id']
            if im_id in self.id2ix:
                self.idx2sg[self.id2ix[im_id]] = _sg
        del(sg)
        
    def read_sg(self, fname):
        print('Loading Scene Graph from {}'.format(fname))
        import json
        f = open(fname, 'r')
        sg = json.load(f)
        f.close()
    ...
    def ix2sg(self, ix):
        ...
        return sg
    def ix2feat(self, ix):
        features = self.dataset.__getitem__(ix)
        #for key
        return features 
...
def getloader(opt):
    dataset = RelationDataset(opt)
    dataset.__getitem__(0)
    dataloader = RelationDataLoader(dataset, opt=opt,
...
class RelationLoader(data.Dataset):
    ...
    def __init__(self, opt):
        self.opt = opt
        self.dataset = RelationDataset(opt)
        tmp = self.dataset.__getitem__(0)
